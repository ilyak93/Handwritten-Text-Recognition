...
def train(opt):
    ...
    """ final options """
    # print(opt)
    with open(f'./saved_models/{opt.exp_name}/opt.txt', 'w', encoding='utf-8') as opt_file:
        opt_log = '------------ Options -------------\n'
        args = vars(opt)
    ...
    while (True):
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0:  # To see training progress, we also conduct validation when 'iteration == 0'
            ...
            # for log
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'w', encoding='utf-8') as log:
                model.eval()
                with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_iter', type=int, default=43000, help='number of iterations to train for')
