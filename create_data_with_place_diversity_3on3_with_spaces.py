...
def create_words_data_3on3_with_spaces(rel_in_path, rel_out_path, map_size, format, images_num):
            ...
            for rws in range(row_shift + 1):
                for c in range(0, cls + 1):
                    for r in range(0, rws + 1):
                        width_start_idx = 0
                        background = np.copy(bg[0:total_length, 0:width*2, :])
                        ...
                        background[c * word_l:(c + 1) * word_l, r * width_step:word_w + r * width_step,
                        :] = cur_word_img
                        background_img = Image.fromarray(background)
                        ImageShow.show(background_img)
                        copies += 1
    ...
    cache['num-samples'.encode()] = str(nSamples).encode()
    writeCache(env, cache)
    print('Created dataset with %d samples of size %d bytes with %d words and %d copies in average' % (
...
if __name__ == '__main__':
    ...
    if not os.path.exists(lmdb_path):
        ...
        words_num = 100000
        print('creating lmdb for train data')
        create_words_data_3on3_with_spaces('/hhd_dataset/train/', './lmdb/train/', 1, 'jpeg', 1000000)
        print('creating lmdb for validation data')
        create_words_data_3on3_with_spaces('/hhd_dataset/val/', './lmdb/val/', 1, 'jpeg', 100000)
